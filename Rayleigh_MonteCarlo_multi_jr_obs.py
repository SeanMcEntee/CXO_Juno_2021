...
try:
    ...
    if (LooseVersion(numba.__version__) > LooseVersion('0.38')):
        use_numba = True
        print()
...
# obsIDs = ['23368', '23369', '23370', '23371', '23372', '23373', '23374', '23375']
# obsIDs = ['23370', '23371']
obsIDs = ['23369']
constraint = 'CML + Lat'
dur = ['355', '355', '352', '201']

start_times = ['2021-09-15 18:29', '2021-09-16 04:26', '2021-09-16 14:24', '2021-09-17 00:20']
end_times = ['2021-09-16 0:24', '2021-09-16 10:2', '2021-09-16 20:1', '2021-09-17 03:41']
...
for count in count_jr:
    for obsID in obsIDs:
        ...
        all = ascii.read(folder_path + f"/{obsID}_photonlist_PI_filter_NE{count}.txt", header_start=0, data_start=0) # updated from Christian Knigge email, previously was data_start=1

        t_all = np.array(all['t(s)'])
        counts_list.append(len(t_all))
        lat = np.array(all['lat (deg)'])
        lon = np.array(all['SIII_lon (deg)'])

        tend = float(ceil((t_all[-1] - t_all[0])/60))        #the end time for N Hot spot 1 for ObsID 2519 from Jackman paper
        ...
        '''if (plotit):
            plt.figure()
            plt.plot(p, pow,'-k', lw=1)
            # plt.scatter(p, pow, s=20, c='k')
            plt.ylabel('Rayleigh Power')
            plt.xlabel('Period (min)')
            plt.title(f'ObsID: {obsID} \n{date}')
            plt.xlim(0, 102)
            plt.tight_layout()'''
            # plt.savefig(f'/Users/mcentees/Desktop/Chandra/Rayleigh/{obsID}_RT_{ntest}.png', dpi=500)
            # plt.show()

            # plt.figure()
            # plt.plot(f, pow,'-k', lw=1)
            # # plt.scatter(f, pow, s=20, c='k')
            # plt.ylabel('Rayleigh Power')
        ...
        for ii in np.arange(ntest):
            ...
            if (ii%1000 == 0) :
                print(f'Done {ii} of {ntest} iterations')
            ...
            if (use_numba):
                ...
                # s2 = np.asarray([ (np.sum(np.sin(omega_i*tf)))**2.0 for omega_i in omega],float,omega.size)
                # c2 = np.asarray([ (np.sum(np.cos(omega_i*tf)))**2.0 for omega_i in omega],float,omega.size)
        ...
        if (plotit):
            ...
            ax[0,0].hist(t, bins, color='black')
            ...
            # full_fig.suptitle(f'ObsID: {obsID} - Jovian Rotation {count}',size =20, y=0.95)
            # full_fig.suptitle(f'ObsID: {obsID} - Northern envelope viewing {count}',size =20, y=0.95)
            full_fig.suptitle(f'Constraint: {constraint}\n{start_times[count-1]} - {end_times[count-1]} ({dur[count-1]} min)',size =16, y=0.95, va='center')
            # full_fig.suptitle(f'ObsID: {obsID} - Southern envelope viewing {count}',size =20, y=0.95)
            ...
            # full_fig.savefig(f'/Users/mcentees/Desktop/Chandra/Rayleigh/{obsID}_RT_{ntest}_all_results_full_obs.png', dpi=500, bbox_inches='tight')
            # full_fig.savefig(f'/Users/mcentees/Desktop/Chandra/Rayleigh/{obsID}_JR{count}_RT_{ntest}_North.png', dpi=500, bbox_inches='tight')
            full_fig.savefig(f'/Users/mcentees/Desktop/paper2/RT_results_lt/Output_plot/{obsID}_NE{count}_{ntest}_runs_title.png', dpi=500, bbox_inches='tight')

output = {'NE Viewing': count_jr, 'Total counts': counts_list, 'Period Corresponding to Max Power': np.round(bestperiod_list, 3), 'Max Power': np.round(maxpower_list, 3), 'P-value': pvalue_list}

out_df = pd.DataFrame(data=output)
# out_df.to_excel(f'/Users/mcentees/Desktop/Chandra/Rayleigh/{obsID}_RT_North_{ntest}.xlsx')
# out_df.to_excel(folder_path + f'/{obsID}_NE_{ntest}_runs.xlsx')
out_df.to_excel(f'/Users/mcentees/Desktop/paper2/RT_results_lt/Output_data/{obsID}_NE_all_{ntest}_runs_title.xlsx')
plt.show()
