...
def plot_xrayctwavelet(time, signal1, scales, datetime, cbarscale, wavelet, waveletname, label,
    ...
    for i in range(len(wavelet)):
        ax = fig.add_subplot(int(len(wavelet)/2), 2, i+1)
        divider = make_axes_locatable(ax)
        # define time steps  
        dt = time[1] - time[0]
        # calculate coefficients and frequencies
        [coefficients, frequencies] = pywt.cwt(signal1, scales, wavelet[i], dt)
        ...
        min_all = 0.900786545338189
        max_all = 5.73657229747919
        ax.set_ylim(max_all, min_all)
        ax.xaxis.set_major_formatter(mdates.DateFormatter("%H:%M"))
        if (i==len(wavelet)-2) or (i==len(wavelet)-1):
        ...
        ax.set_title(r'$\bf{%s}$' % label[i] + f' Northern Aurora {waveletname[i]} Wavelet Power Spectrum')
    fig.tight_layout()
    ...
    time_inv = time_bins[np.where(np.array(max_log_power) > 3.)[0]]
    datetime_res = datetime[np.where(np.array(max_log_power) > 3.0)[0]]
    ...
    datetime_ints_st = [datetime_res[i + 1] for i in ints]
    datetime_ints_end = [datetime_res[i] for i in ints]

    ax1.axvline(datetime_res[0], 0, 10, color='limegreen', lw=2, zorder=10)
    ax1.axvline(datetime_res[-1], 0, 10, color='limegreen', lw=2, zorder=10)
    ax2.axvline(datetime_res[0], 0, 1, color='limegreen', lw=2, zorder=10)
    ax2.axvline(datetime_res[-1], 0, 1, color='limegreen', lw=2, zorder=10)

    for i in range(len(datetime_ints_st)):
        ax1.axvline(datetime_ints_st[i], 0, 10, color='limegreen', lw=2, zorder=10)
        ax1.axvline(datetime_ints_end[i], 0, 10, color='limegreen', lw=2, zorder=10)
        ax2.axvline(datetime_ints_st[i], 0, 1, color='limegreen', lw=2, zorder=10)
        ax2.axvline(datetime_ints_end[i], 0, 1, color='limegreen', lw=2, zorder=10)

    return power, period, coefficients, datetime_res, ints, datetime_ints_st, datetime_ints_end'''
...
plt.savefig(outdir + f'/{obsID}_multiple_wavelets_lt.png', dpi=500, bbox_inches='tight')
plt.show()
